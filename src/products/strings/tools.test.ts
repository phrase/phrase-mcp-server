...
describe("strings tools", () => {
  ...
  it("strings_create_upload includes API error message for JSON message strings", async () => {
    calls = [];
    registerWithClient(
      createClientProxy(calls, async (methodPath) => {
        if (methodPath === "uploadsApi.uploadCreate") {
          return Promise.reject(
            new Response(JSON.stringify({ message: "bad upload request" }), {
              status: 422,
    ...
    const registration = registrations.get("strings_create_upload");
    expect(registration).toBeDefined();
    if (!registration) {
      return;
    }
    ...
    const input = buildRequiredInput(registration.inputSchema, uploadFilePath);
    await expect(registration.handler(input)).rejects.toThrow(
      "Phrase Strings request failed (422 Unprocessable Entity): bad upload request",
    );
  });

  it("strings_create_upload includes joined API error messages for JSON arrays", async () => {
    calls = [];
    registerWithClient(
      createClientProxy(calls, async (methodPath) => {
        if (methodPath === "uploadsApi.uploadCreate") {
          return Promise.reject(
            new Response(JSON.stringify({ message: ["first problem", "second problem"] }), {
              status: 400,
    ...
    const input = buildRequiredInput(registration.inputSchema, uploadFilePath);
    await expect(registration.handler(input)).rejects.toThrow(
      "Phrase Strings request failed (400 Bad Request): first problem; second problem",
    );
  });
});
