...
export function registerGetJobTemplateTool(server: McpServer, runtime: ProductRuntime) {
  server.registerTool(
    ...
    async ({ project_id, id, branch }) => {
      const jobTemplate = await (runtime.client as StringsClient).jobTemplatesApi.jobTemplatesShow({
