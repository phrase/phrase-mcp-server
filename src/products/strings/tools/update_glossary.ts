...
export function registerGlossaryUpdateTool(server: McpServer, runtime: ProductRuntime) {
  server.registerTool(
    ...
    async ({ account_id, id, name, project_ids, space_ids }) => {
      const glossary = await (runtime.client as StringsClient).glossariesApi.glossaryUpdate({
