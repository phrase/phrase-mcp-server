    ...
    async ({ config_path }) => {
      ...
      const getEffectiveProjectId = async (branch?: string): Promise<string> => {
        ...
        if (!branchProjectIdCache.has(branch)) {
          const branchInfo = await runtime.client.branchesApi.branchShow({
            projectId,
            name: branch,
          });
          branchProjectIdCache.set(
            branch,
            (branchInfo as { branchProjectId?: string }).branchProjectId ?? projectId,
          );
        }
        return branchProjectIdCache.get(branch) ?? projectId;
      };

      // Cache locale lists per branch
      const localesCache = new Map<
        string,
        Awaited<ReturnType<typeof runtime.client.localesApi.localesList>>
      >();
      const getLocales = async (effectiveProjectId: string) => {
        if (!localesCache.has(effectiveProjectId)) {
          localesCache.set(
            effectiveProjectId,
            await runtime.client.localesApi.localesList({ projectId: effectiveProjectId }),
          );
        }
      ...
      for (const target of targets) {
        ...
        for (const locale of localesToDownload) {
          const resolvedFile = resolve(
            configDir,
            substitutePlaceholders(target.file, {
              name: locale.name ?? "",
              code: locale.code ?? "",
            }),
          );
