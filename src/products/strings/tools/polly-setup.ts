...
export function createPolly(cassetteName: string): Polly {
  ...
  polly.server.any().on("beforePersist", (_req, recording) => {
    ...
    // Scrub session cookies from response Set-Cookie headers and cookie objects
    for (const header of recording.response.headers ?? []) {
      if ((header.name as string).toLowerCase() === "set-cookie") {
        header.value = (header.value as string).replace(
          /_phrase_session_sec=[^;]*/,
          "_phrase_session_sec=REDACTED",
        );
      }
    }
    for (const cookie of (recording.response.cookies as Array<{ name: string; value: string }>) ??
      []) {
      if (cookie.name === "_phrase_session_sec") {
