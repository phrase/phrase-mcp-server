...
export function registerCreateAnalysesByLanguagesTool(
  server: McpServer,
  ...
  server.registerTool(
    "tms_create_analyses_by_languages",
    {
      ...
      annotations: { title: "[TMS] Create Analyses by Languages", destructiveHint: true },
      inputSchema: {
        jobs: z
    ...
    async (params) => {
      ...
      const result = await runtime.client.postJson("/v1/analyses/byLanguages", payload);
