...
export function registerSetProjectStatusTool(server: McpServer, runtime: ProductRuntime) {
  server.registerTool(
    "tms_set_project_status",
    {
      ...
      inputSchema: {
        ...
        status: z
          .string()
          .min(1)
          .describe("Target project status value accepted by TMS (for example NEW, COMPLETED, CANCELLED)."),
      },
    },
    async ({ project_uid, status }) => {
      const response = await (runtime.client as TmsClient).postJson(
